...
Channel adapter 為**薄層**，核心邏輯獨立部署於 `/data/personal/` 下各自的服務，adapter 透過 HTTP API 消費。

目前實作：

| Channel | 子類型 | Inbound | Outbound | 核心服務位置 |
|---------|--------|---------|----------|------------|
| LINE 個人帳號 | `line/personal/` | ✅（已獨立） | ✅（已獨立） | `/data/personal/line-personal/`（FastAPI :8000） |
| LINE 官方帳號 | `line/official/` | ⬜ | ⬜ | `/data/personal/line-official/`（搬遷中） |

### 服務分離原則

LINE 的實際通訊能力（CDP、Messaging API、E2EE、媒體）均獨立為 `/data/personal/` 下的服務：

- **line-personal**：Chrome CDP + PostgreSQL + FastAPI，長駐 daemon，對外暴露 REST API
- **line-official**：Messaging API webhook server + push/reply（搬遷中，原位於 `src/channels/line/official/`）

Switchboard channel adapter 不包含平台邏輯，只做：
1. 呼叫對應服務 API 取得訊息
2. 將回應轉換為 switchboard 統一 Message schema
