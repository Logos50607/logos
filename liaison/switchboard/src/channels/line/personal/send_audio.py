...
# ///
"""
send_audio.py - 透過 LINE GW API（E2EE V2）發送音訊訊息

流程：
  1. 生成 32-byte keyMaterial（隨機）
  2. HKDF(SHA-256, km, "FileEncryption") → encKey / macKey / nonce
  3. AES-CTR 加密 + single HMAC-SHA256（非 chunked）
  4. POST 加密音訊至 OBS（/r/talk/ema/reqid-<rand>）
  5. 從回應標頭取得 x-obs-oid
  6. E2EE 加密 {"keyMaterial": km_b64} 為 chunks
  7. 呼叫 sendMessage API（contentType=3, SID=ema, DURATION=ms）

建議格式：m4a（LINE extension 內部轉換目標格式）
用法：
  uv run send_audio.py --to <mid> --file audio.m4a
"""

# ── 目錄 ─────────────────────────────────────────────────────────
# 1. _get_duration_ms(path)       - 用 av 讀取音訊時長
# 2. _encrypt_audio(data, km_b64) - HKDF + AES-CTR + single HMAC
# 3. _obs_upload(...)             - POST 加密音訊至 OBS /r/talk/ema/
# 4. _build_send_body(...)        - 組 sendMessage body
# 5. send_audio(page, to, path)   - 完整流程
# 6. main（standalone）
...
_CT_AUDIO  = 3   # EU.AUDIO


# ── 1. 讀取音訊時長 ───────────────────────────────────────────────

def _get_duration_ms(file_path: Path) -> int:
    """用 PyAV 讀取音訊時長（毫秒），失敗回傳 0。"""
    try:
        ...
        print(f"    無法讀取時長（{e}），使用 0", flush=True)
    return 0


# ── 2. 加密 ───────────────────────────────────────────────────────

def _encrypt_audio(data: bytes, km_b64: str) -> bytes:
...
def _build_send_body(seq_num: int, to: str, my_mid: str,
                     ...
                     chunks: list) -> list:
    """組 sendMessage API body（contentType=3 音訊）。"""
    return [seq_num, {
        "from": my_mid, "to": to, "toType": 0,
        "id": f"local-{seq_num}", "contentType": _CT_AUDIO,
        "contentMetadata": {
            "e2eeVersion": "2",
            "SID": "ema",
            "OID": oid,
...
async def send_audio(page, to: str, file_path: Path) -> dict:
    """
    發送 E2EE V2 音訊訊息。
    ...
    print(">>> 讀取音訊時長...", flush=True)
    dur_ms = _get_duration_ms(file_path)
    print(f"    時長: {dur_ms}ms", flush=True)
    ...
    print(">>> 加密音訊...", flush=True)
    enc_data = _encrypt_audio(data, km_b64)
    ...
    chunks    = await encrypt_message(page, to, my_mid,
                                      ...
                                      plaintext, content_type=_CT_AUDIO)

    print(">>> 發送訊息...", flush=True)
    body_obj = _build_send_body(seq_num, to, my_mid, oid, file_size, dur_ms, chunks)
...
async def main():
    p = argparse.ArgumentParser(description="Send LINE audio via GW API (E2EE V2)")
    p.add_argument("--to",   required=True, help="目標 mid")
    p.add_argument("--file", required=True, help="音訊路徑（建議 m4a）")
    args = p.parse_args()
    ...
    async with async_playwright() as pw:
        ...
        result = await send_audio(page, args.to, file_path)
        if result.get('ok'):
