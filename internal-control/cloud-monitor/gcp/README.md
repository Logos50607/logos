...
| `real-asset-sale-ricky` | Cloud Run（real-asset-sale-kit） | 啟用 | self |
...
| 每日費用摘要推播 | **已部署**（stub 模式，等 OA ready 填 token） | `daily-summary` CF |
| 部署事件通知 | **已部署**（stub 模式，等 OA ready 填 token） | `deploy-notify` CF |
| 請款分流 | 藍圖 | - |
