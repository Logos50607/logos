...
<!-- START_FILE: discipline_metadata.md (MD5: 2bd5fe88da6e9675437e25227acf7e9b) -->
---
...
<!-- START_FILE: fp_design.md (MD5: ca0bdf67a53bf3218c60b7a4047498f7) -->
---
name: fp_design
trigger: always_on
description: "函數式設計原則，規範純函式分離、統一介面與依賴注入，以確保程式碼可單元測試。"
---

# 函數式設計原則 (Functional Programming Design)

核心思路：**Functional Core, Imperative Shell** — 純邏輯與 I/O 嚴格分層。

## 1. 資料與轉換分離

- 轉換邏輯（filter、map、score、transform）必須實作為**純函式**，不得在內部觸碰 I/O。
- 純函式**必須 export**，讓單元測試可直接 import，不得藏在 handler 或 class 私有方法中。
- I/O（網路、資料庫、檔案、瀏覽器）只允許出現在最外層殼層（entry point）。

## 2. 依賴注入

- Orchestration 函式（service layer）不直接呼叫 I/O 實作，改以**函式參數（deps）**接收。
- 外層殼層負責注入真實實作；測試注入 mock。

## 3. 統一介面

- 功能相似的模組**必須實作相同介面**（相同 input shape、相同 output shape）。
- 模組間的差異（欄位名稱、資料格式）封裝在各自實作內，上層 pipeline 不感知。
...
|------|---------|
| 純函式層 | 單元測試，無需 mock |
| Service layer | 注入 mock deps 測試 orchestration 邏輯 |
| I/O / Entry point | 不寫單元測試，以 e2e 或手動驗收 |
<!-- END_FILE: fp_design.md -->
