...
在步驟上方加上 `// turbo` 註解，可以指示 AI 在執行該步驟（如
`run_command`）時自動執行 (Set `SafeToAutoRun` to true)，無需額外詢問使用者。

- **`// turbo`**: 僅應用於下方緊鄰的一個步驟。
- **`// turbo-all`**: 應用於該工作流內的所有步驟。
...
- **腳本模組化**: 工作流所呼叫的底層腳本應維持在 **100 行以內**。若功能過於龐大應主動切分。
- **索引導航**: 腳本應扮演 **Index / Content List** 角色，引導 AI 操作特定邏輯。推薦使用 **Pipe (管道)** 或 **Strategy Pattern (策略模式)** 以注入方式驅動高階邏輯。
